  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (!hold)
        return reply
          .code(404)
          .send({ ok: false, error: "Hold tidak ditemukan" });
      return reply.send({ ok: true, data: hold });
    }
  );
    ...
    async (req, reply) => {
      ...
      if ((hold as any).status && (hold as any).status !== "DRAFT") {
        return reply
          ...
          .send({
            ok: false,
            error: `Tidak bisa hapus hold dengan status ${
              (hold as any).status
            }`,
          });
      }
    ...
    async (req, reply) => {
      ...
      if (!kasir) {
        return reply
          ...
          .send({
            ok: false,
            error: `Kasir tidak ditemukan: ${hold.cashierId}`,
          });
      }
      ...
      if (hold.customerId) {
        ...
        if (!cust) {
          return reply
            ...
            .send({
              ok: false,
              error: `Customer tidak ditemukan: ${hold.customerId}`,
            });
        }
      }
      ...
      const productIds = Array.from(
        new Set(payload.items.map((i: any) => i.productId))
      );
      ...
      const getToBase = (pid: string, uom: string) =>
      ...
      const subtotal = payload.items.reduce(
        (s: number, it: any) => s + (it.qty * it.price - (it.discount ?? 0)),
        0
      );
      const total = Math.max(0, subtotal - (payload.discountTotal ?? 0));
      const paid = payload.payments.reduce(
